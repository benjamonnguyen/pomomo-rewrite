...
export const execute = async (interaction) => {
    ...
    if (!focusMember) {
        ...
        await Promise.all([
            focusMemberRepo.add(focusMember.guildId, focusMember.channelId, interaction.user.id),
            focusMemberRepo.set(interaction.user.id, focusMember),
        ]);
    }
    else {
        try {
            const oldMsg = interaction.channel.messages.cache.get(focusMember.messageId);
            if (oldMsg) {
                await oldMsg.delete();
            }
        ...
        await focusMemberRepo.remove(focusMember.guildId, focusMember.channelId, interaction.user.id);
    ...
    await Promise.allSettled([
        ...
        sessionRepo
            ...
            .then((session) => handleAutoshush(session, interaction.guild.members, new Set([interaction.user.id]))),
    ]);
};
